...
$(document).ready(function() {
  ...
  $("#speciality_id_e").change(function(event) { //carga los doctores en el select #doctor_id según la especialidad elegida.
    ...
    if (!id) $("#doctor_id_e").html("<option>--Seleccione--</option>")
  ...
  $("#speciality_id").change(function(event) { //carga los doctores en el select #doctor_id según la especialidad elegida.
    var id = event.target.value;
    if (!id) $("#doctor_id").html("<option>--Seleccione--</option>")
    $('.selectpicker').selectpicker('refresh') //refresca el select para que cambie su valor
    $.get("get-doctor/" + id + "", function(response, speciality) {
      $('.selectpicker').selectpicker('refresh') //refresca el select para que cambie su valor
      $("#doctor_id").empty()
...
function del_paciente(id) { // elimina un paciente
  var route = "/pacientes/" + id + "";
  var csrf_token = $('meta[name="csrf-token"]').attr('content');
  $.ajax({
    url: route,
...
function del_especialidad(id) { // elimina un paciente
  var route = "/especialidades/" + id + "";
  var csrf_token = $('meta[name="csrf-token"]').attr('content');
  $.ajax({
    url: route,
...
function del_rol(id) { // elimina un paciente
  var route = "/roles/" + id + "";
  var csrf_token = $('meta[name="csrf-token"]').attr('content');
  $.ajax({
    url: route,
...
function del_permiso(id) { // elimina un paciente
  var route = "/permisos/" + id + "";
  var csrf_token = $('meta[name="csrf-token"]').attr('content');
  $.ajax({
    url: route,
...
function del_doctor(id) { // elimina un paciente
  var route = "/doctores/" + id + "";
  var csrf_token = $('meta[name="csrf-token"]').attr('content');
  $.ajax({
    url: route,
...
function del_recepcionista(id) { // elimina un paciente
  var route = "/recepcionistas/" + id + "";
  var csrf_token = $('meta[name="csrf-token"]').attr('content');
  $.ajax({
    url: route,
...
function atender(id) { //Carga mestra el modal para realizar una atención.
...
function delete_cita_pendiente(id) { //esta funcion elimina una cita oendiente desde el modulo de consultas medicas.
  var popup = confirm("¿ Esta seguro de eliminar esta cita ?")
  var route = "/citas/" + id + "";
  var csrf_token = $('meta[name="csrf-token"]').attr('content');
  if (popup == true) {
